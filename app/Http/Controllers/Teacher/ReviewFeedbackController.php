...
class ReviewFeedbackController extends Controller
{
    ...
    {
        $validated = $request->validate(['review'=>['nullable',Rule::in(Lesson::REVIEW_STATUSES)],'q'=>['nullable','string','max:120']]);
        $baseQuery = Lesson::query()->where('status','published')->whereHas('unit.course', fn($query)=>$query->where('created_by',$request->user()->id));
        $lessonsQuery = (clone $baseQuery)->with(['unit.course.subject','reviewer']);

        if (! empty($validated['review'])) $lessonsQuery->where('review_status',$validated['review']);
        if (! empty($validated['q'])) {
            $term=$validated['q'];
            $lessonsQuery->where(function($query) use($term){$query->where('title','like',"%{$term}%")->orWhereHas('unit',fn($unitQuery)=>$unitQuery->where('title','like',"%{$term}%"))->orWhereHas('unit.course',fn($courseQuery)=>$courseQuery->where('title','like',"%{$term}%"));});
        }

        $lessons=$lessonsQuery->orderByRaw("CASE review_status WHEN 'changes_requested' THEN 0 WHEN 'pending' THEN 1 ELSE 2 END")->orderByDesc('reviewed_at')->orderByDesc('updated_at')->paginate(15)->withQueryString();
    ...
    public function resubmit(Request $request, Lesson $lesson, ReviewAuditLogger $audit): RedirectResponse
    {
        ...
        abort_unless($lesson->unit?->course?->created_by === $request->user()->id, 403);
        if (! $lesson->changesRequested()) throw ValidationException::withMessages(['teacher_response'=>'Only a Lesson with Changes Requested can be resubmitted.']);
        ...
        Notification::send($admins,new ReviewWorkflowNotification(
            ...
            $request->user()->name.' resubmitted “'.$lesson->title.'” after addressing review feedback.',
            route('admin.curriculum.courses.show',$lesson->unit->course_id),
            $lesson->id
        ));

        return redirect()->route('teacher.reviews.index',['review'=>'pending'])->with('status','Lesson resubmitted for Admin review.');
    }
}
