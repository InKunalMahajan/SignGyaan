...
const enhanceForms = () => {
    ...
    if (errorSummary instanceof HTMLElement) {
        ...
        const reducedMotionPreference = document.body?.dataset.reducedMotion === 'on';
...
const enhanceLessonVideoProgress = () => {
    ...
    if (!(config instanceof HTMLElement) || !(video instanceof HTMLVideoElement)) {
        return;
    }
    ...
    video.addEventListener('loadedmetadata', () => {
        if (
            ...
            if (status instanceof HTMLElement) {
                status.textContent = `Video resumed at ${formatVideoTime(savedPosition)}.`;
            }
        }
    });

    const savePosition = async (force = false) => {
        if (saving || !Number.isFinite(video.duration) || video.duration <= 0) {
            return;
        }

        const now = Date.now();

        if (!force && (now - lastSaveAt < 15000 || Math.abs(video.currentTime - lastSavedPosition) < 5)) {
            return;
        }
        ...
        try {
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    subject_slug: config.dataset.subject,
                    course_slug: config.dataset.course,
                    lesson_id: Number(config.dataset.lessonId),
            ...
            if (status instanceof HTMLElement) {
                status.textContent = `Video position saved at ${formatVideoTime(lastSavedPosition)}. Lesson completion remains manual.`;
            }

            if (label instanceof HTMLElement) {
                label.textContent = `Video watched: ${data.watched_percent}%`;
            }
        } catch {
            if (status instanceof HTMLElement) {
                status.textContent = 'Video position could not be saved right now. Lesson completion is unchanged.';
            }
        } finally {
            saving = false;
        }
