...
@section('content')
    ...
    <div data-learner-dashboard>
        <section class="border-b border-sign-border bg-sign-soft py-8 sm:py-10 lg:py-12" aria-labelledby="dashboard-heading">
            <x-container>
                <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-center">
                    <div>
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <h1 id="dashboard-heading" class="mt-1 font-heading text-3xl font-semibold tracking-tight text-sign-primary sm:text-4xl lg:text-5xl">Welcome, {{ $firstName }}.</h1>
                            </div>
                        </div>
                    ...
                    <div class="rounded-2xl border border-sign-border bg-white p-5 shadow-sm sm:rounded-3xl" aria-label="Continue quickly">
        ...
        <div class="sticky top-16 z-30 border-b border-sign-border bg-white/95 backdrop-blur sm:top-20 lg:static lg:bg-white" aria-label="Dashboard shortcuts">
            <x-container>
                <div data-dashboard-quick-nav class="-mx-1 flex gap-2 overflow-x-auto px-1 py-3 [scrollbar-width:thin] lg:flex-wrap lg:overflow-visible" tabindex="0">
                    <a href="#dashboard-continue" class="inline-flex shrink-0 items-center rounded-full border border-sign-border bg-white px-4 py-2 text-sm font-semibold text-sign-primary hover:border-sign-cyan hover:bg-sign-soft">Continue</a>
                    @if (($recommendedNextLessons ?? collect())->isNotEmpty())
                        <a href="#recommended-next-lessons-heading" class="inline-flex shrink-0 items-center rounded-full border border-sign-border bg-white px-4 py-2 text-sm font-semibold text-sign-primary hover:border-sign-cyan hover:bg-sign-soft">Recommended</a>
                    @endif
                    <a href="#assessment-progress-heading" class="inline-flex shrink-0 items-center rounded-full border border-sign-border bg-white px-4 py-2 text-sm font-semibold text-sign-primary hover:border-sign-cyan hover:bg-sign-soft">Assessments</a>
                    @isset($activitySummary)
                        <a href="#learning-activity-heading" class="inline-flex shrink-0 items-center rounded-full border border-sign-border bg-white px-4 py-2 text-sm font-semibold text-sign-primary hover:border-sign-cyan hover:bg-sign-soft">Activity</a>
                    @endisset
                </div>
            </x-container>
        </div>

        <section class="bg-white py-10 sm:py-14 lg:py-16" aria-labelledby="dashboard-overview-heading">
            <x-container>
                <h2 id="dashboard-overview-heading" class="sr-only">Learning overview</h2>
                ...
                <div class="mt-10 grid gap-8 xl:grid-cols-[minmax(0,1fr)_19rem] xl:items-start">
                    ...
                    <aside class="space-y-5 xl:sticky xl:top-24" aria-label="Dashboard quick actions">
                        <div class="rounded-2xl border border-sign-border bg-sign-soft p-5 sm:rounded-3xl">
                            ...
                            <nav class="mt-4 space-y-2" aria-label="Learning links">
                                ...
                                <a href="{{ route('my-courses') }}" class="flex min-h-11 items-center justify-between rounded-xl bg-white px-4 py-3 text-sm font-semibold text-sign-primary hover:bg-sign-light"><span>My Courses</span><span aria-hidden="true">→</span></a>
                                <a href="{{ route('learning-history') }}" class="flex min-h-11 items-center justify-between rounded-xl bg-white px-4 py-3 text-sm font-semibold text-sign-primary hover:bg-sign-light"><span>Learning History</span><span aria-hidden="true">→</span></a>
                        ...
                        @if ($assessmentSummary['submitted'] > 0)
                            <div class="rounded-2xl border border-sign-border bg-white p-5 sm:rounded-3xl">
                                ...
                                <a href="{{ route('assessment-performance') }}" class="mt-4 inline-flex min-h-11 items-center text-sm font-semibold text-sign-primary hover:text-sign-cyan-dark">View performance →</a>
                            </div>
                        @endif
