...
@section('content')
<section class="px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10" data-admin-assessment-results>
    <div class="mx-auto max-w-7xl">
        ...
        <div class="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Assessment performance summary">
            @foreach ([
        ...
        <form method="GET" action="{{ route('admin.assessment-results.index') }}" class="mt-7 rounded-2xl border border-sign-border bg-white p-4 sm:rounded-3xl sm:p-5" role="search" aria-label="Filter assessment results">
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_13rem_12rem_12rem_12rem_auto] xl:items-end">
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="submit" class="inline-flex min-h-12 items-center justify-center rounded-xl bg-sign-primary px-5 py-3 text-sm font-semibold text-white hover:bg-sign-dark">Filter</button>
        ...
        <div class="mt-7 overflow-hidden rounded-2xl border border-sign-border bg-white sm:rounded-3xl">
            @if ($attempts->count())
                <div class="overflow-x-auto" tabindex="0" aria-label="Scrollable learner assessment results table">
                    <table class="min-w-full divide-y divide-sign-border text-left text-sm">
                        <caption class="sr-only">Learner assessment attempts with scores, status and result detail links.</caption>
                        <thead class="bg-sign-soft text-xs font-semibold uppercase tracking-wider text-sign-muted">
                            <tr>
                                <th scope="col" class="px-5 py-4">Learner</th>
                                <th scope="col" class="px-5 py-4">Assessment</th>
                                <th scope="col" class="px-5 py-4">Attempt</th>
                                <th scope="col" class="px-5 py-4">Score</th>
                        ...
                        <tbody class="divide-y divide-sign-border">
                            @foreach ($attempts as $attempt)
                                ...
                                <tr class="align-top">
                                    ...
                                    <td class="px-5 py-4 text-right"><a href="{{ route('admin.assessment-results.show', $attempt) }}" class="inline-flex min-h-10 items-center justify-center rounded-lg border border-sign-border px-3 py-2 text-xs font-semibold text-sign-primary hover:bg-sign-soft" aria-label="View details for {{ $attempt->user?->name ?? 'learner' }}, attempt {{ $attempt->attempt_number }}">View Details</a></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
