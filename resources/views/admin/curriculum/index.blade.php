...
<body class="min-h-screen bg-slate-50 text-slate-950 antialiased">
    <main class="mx-auto max-w-7xl space-y-6 px-5 py-8 sm:px-8">
        ...
        <form method="GET" class="grid gap-3 rounded-2xl border bg-white p-5 sm:grid-cols-[1fr_220px_auto]">
            ...
            <select name="review" class="rounded-xl border border-slate-300 px-4 py-3">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse ($courses as $course)
                <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex items-start justify-between gap-3"><div><p class="text-xs font-black uppercase text-cyan-700">{{ $course->subject?->name ?: 'No subject' }}</p><h2 class="mt-1 text-lg font-black">{{ $course->title }}</h2></div><span class="rounded-full px-2.5 py-1 text-xs font-black {{ $course->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $course->is_active ? 'Active' : 'Inactive' }}</span></div>
                    <p class="mt-3 text-sm text-slate-500">Teacher: {{ $course->creator?->name ?: 'Unknown' }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ $course->units_count }} units</p>
                    <a href="{{ route('admin.curriculum.courses.show', $course) }}" class="mt-5 inline-flex rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-black text-white">Review Course</a>
                </article>
            @empty
