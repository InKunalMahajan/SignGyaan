    ...
    <header class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-8">
            ...
            <nav class="flex flex-wrap items-center gap-2" aria-label="Admin navigation">
                ...
                <a href="{{ route('admin.curriculum.index') }}" class="rounded-xl px-3 py-2 text-sm font-bold {{ request()->routeIs('admin.curriculum.index') || request()->routeIs('admin.curriculum.boards.*') || request()->routeIs('admin.curriculum.classes.*') || request()->routeIs('admin.curriculum.subjects.*') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100' }} focus:outline-none focus:ring-4 focus:ring-indigo-200">Catalog</a>
                <a href="{{ route('admin.curriculum.content.index') }}" class="rounded-xl px-3 py-2 text-sm font-bold {{ request()->routeIs('admin.curriculum.content.*') ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100' }} focus:outline-none focus:ring-4 focus:ring-indigo-200">Course Content</a>
