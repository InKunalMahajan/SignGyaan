...
@section('description', 'View saved SignGyaan course progress, continue lessons and review completed learning.')
...
@section('content')
    @php
        ...
        $starterCourses = [
            ['title' => 'English Foundations', 'subject' => 'English', 'level' => 'Beginner', 'description' => 'Build vocabulary, grammar and simple sentence understanding.', 'url' => route('courses.show', ['subject' => 'english', 'course' => 'english-foundations', 'lesson' => 'unit-1-lesson-1'])],
            ['title' => 'Computer Basics', 'subject' => 'Digital Skills', 'level' => 'Beginner', 'description' => 'Learn hardware, software, files, folders and everyday computer use.', 'url' => route('courses.show', ['subject' => 'digital-skills', 'course' => 'computer-basics', 'lesson' => 'unit-1-lesson-1'])],
            ['title' => 'Everyday Mathematics', 'subject' => 'Mathematics', 'level' => 'Beginner', 'description' => 'Use numbers, money, time and calculations in practical situations.', 'url' => route('courses.show', ['subject' => 'mathematics', 'course' => 'everyday-mathematics', 'lesson' => 'unit-1-lesson-1'])],
        ];
    @endphp
        ...
        <x-container>
            ...
            <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-4 max-w-2xl text-sm leading-7 text-sign-muted sm:text-base">Welcome, {{ $firstName }}. Your saved courses, completed lessons and next learning position are stored here.</p>
                </div>
        ...
        <x-container>
            <div class="grid gap-4 sm:grid-cols-3">
                ...
                <div class="rounded-2xl border border-sign-border p-5 sm:rounded-3xl sm:p-6">
                    ...
                    <p class="mt-1 text-xs leading-5 text-sign-muted">Saved across {{ $progressRecords->count() }} {{ Str::plural('course', $progressRecords->count()) }}.</p>
                </div>
            </div>
                    ...
                    <section aria-labelledby="my-learning-continue-heading">
                                    ...
                                    <article class="overflow-hidden rounded-2xl border border-sign-border bg-white sm:rounded-3xl">
                                        <div class="grid lg:grid-cols-[minmax(0,1fr)_16rem]">
                                            <div class="p-5 sm:p-6">
                                                ...
                                                <p class="mt-2 text-sm leading-6 text-sign-muted">{{ $progress->completedLessonsCount() }} of {{ $progress->total_lessons }} lessons completed. Continue from your saved position.</p>
                                            ...
                                            <div class="flex items-center bg-sign-soft p-5 sm:p-6">
                                                <div class="w-full">
                                                    <p class="text-xs font-semibold uppercase tracking-wider text-sign-cyan-dark">Saved position</p>
                                                    <p class="mt-2 break-words text-sm font-semibold text-sign-primary">{{ str_replace('-', ' ', Str::headline($progress->current_lesson_key)) }}</p>
                        ...
                        @else
                            <div class="mt-5 rounded-2xl border border-dashed border-sign-border bg-sign-soft p-6 sm:rounded-3xl sm:p-8">
                                ...
                                <p class="mt-2 max-w-2xl text-sm leading-6 text-sign-muted">Start a lesson, then use Save My Place or Complete & Continue. Your progress will appear here.</p>
                            </div>
                        @endif
                    </section>
                ...
                <aside class="space-y-5 xl:sticky xl:top-24" aria-label="My Learning navigation">
                    <div class="rounded-2xl border border-sign-border bg-sign-soft p-5 sm:rounded-3xl">
                        ...
                        <div class="mt-4 space-y-3 text-sm">
                            <div class="flex items-center justify-between rounded-xl bg-white px-4 py-3"><span class="text-sign-muted">In Progress</span><span class="font-semibold text-sign-primary">{{ $activeCourses->count() }}</span></div>
                            <div class="flex items-center justify-between rounded-xl bg-white px-4 py-3"><span class="text-sign-muted">Completed</span><span class="font-semibold text-sign-primary">{{ $completedCourses->count() }}</span></div>
                    ...
                    <div class="rounded-2xl border border-sign-border bg-white p-5 sm:rounded-3xl">
                        ...
                        <nav class="mt-4 space-y-2">
                            <a href="{{ route('subjects') }}" class="flex min-h-11 items-center justify-between rounded-xl bg-sign-soft px-4 py-3 text-sm font-semibold text-sign-primary hover:bg-sign-light"><span>Browse Subjects</span><span>→</span></a>
                            <a href="{{ route('explore') }}" class="flex min-h-11 items-center justify-between rounded-xl bg-sign-soft px-4 py-3 text-sm font-semibold text-sign-primary hover:bg-sign-light"><span>Explore Learning</span><span>→</span></a>
                        </nav>
                    </div>
                </aside>
            </div>
