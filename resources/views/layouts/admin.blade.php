        ...
        <header class="sticky top-0 z-30 border-b border-sign-border bg-white/95 backdrop-blur">
            <div class="flex min-h-16 items-center gap-3 px-4 sm:min-h-20 sm:px-6 lg:px-8">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs font-semibold uppercase tracking-wider text-sign-cyan-dark">SignGyaan Admin</p>
                    <h1 class="truncate font-heading text-lg font-semibold text-sign-primary sm:text-xl">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    <a
                        href="{{ route('home') }}"
                        class="hidden min-h-11 items-center justify-center rounded-xl border border-sign-border bg-white px-4 py-2.5 text-sm font-semibold text-sign-primary transition hover:border-sign-cyan hover:bg-sign-soft sm:inline-flex"
                    >
                        View Website
                    </a>

                    @php
                        $adminLayoutUser = auth()->user();
                        $adminLayoutInitial = strtoupper(substr(trim($adminLayoutUser->name), 0, 1));
                    @endphp
                    ...
                    >
                        ...
                        <span class="hidden max-w-36 truncate text-sm font-semibold text-sign-primary md:block">{{ $adminLayoutUser->name }}</span>
                    </a>
                </div>
            </div>
        </header>
