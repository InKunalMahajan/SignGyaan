...
<head>
    ...
    <meta name="description" content="@yield('meta_description', 'SignGyaan parent workspace')">
    <title>@yield('title', 'Parent') | SignGyaan</title>
    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside class="border-b border-slate-200 bg-white lg:min-h-screen lg:border-b-0 lg:border-r">
            <div class="px-5 py-5 lg:px-6">
                <a href="{{ route('dashboard.role', 'parents') }}" class="flex items-center gap-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900" aria-label="SignGyaan parent dashboard">
            ...
            <nav class="px-4 pb-5 lg:px-5" aria-label="Parent navigation">
                ...
                <div class="flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible">
                    <a href="{{ route('dashboard.role', 'parents') }}" class="flex min-w-fit items-center rounded-xl px-3 py-3 text-sm font-bold {{ request()->routeIs('dashboard.role') ? 'bg-slate-950 text-white' : 'text-slate-700 hover:bg-slate-100' }}">Dashboard</a>
                    <a href="{{ route('parents.profile.show') }}" class="flex min-w-fit items-center rounded-xl px-3 py-3 text-sm font-bold {{ request()->routeIs('parents.profile.*') ? 'bg-slate-950 text-white' : 'text-slate-700 hover:bg-slate-100' }}">Parent Profile</a>
                    <a href="{{ route('search.index') }}" class="flex min-w-fit items-center rounded-xl px-3 py-3 text-sm font-bold {{ request()->routeIs('search.*') ? 'bg-slate-950 text-white' : 'text-slate-700 hover:bg-slate-100' }}">Search</a>
            ...
            <div class="mx-5 mb-5 hidden rounded-xl border border-slate-200 bg-slate-50 p-4 lg:block">
                ...
                <p class="mt-1 text-xs font-semibold text-slate-500">Parent</p>
        ...
        <main id="main-content" class="min-w-0">
            <header class="border-b border-slate-200 bg-white px-5 py-4 sm:px-8 lg:px-10">
                <div class="mx-auto flex max-w-7xl items-center justify-between gap-4">
                    <div>
                        <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Family Learning</p>
                        <p class="mt-1 text-sm font-semibold text-slate-500">@yield('header_label', 'Parent')</p>
                    </div>
