...
<body class="min-h-screen bg-slate-50 text-slate-950 antialiased">
        ...
        <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-8">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 rounded-xl focus:outline-none focus:ring-4 focus:ring-cyan-200">
                ...
                <span>
                    ...
                    <span class="block text-xs font-semibold text-slate-500">Teacher Workspace</span>
                </span>
            </a>
            ...
            <nav class="flex flex-wrap items-center gap-2" aria-label="Teacher workspace">
                ...
                <a href="{{ route('teacher.reviews.index') }}" class="rounded-xl px-3 py-2 text-sm font-black text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-4 focus:ring-cyan-200">Review Feedback</a>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-4 focus:ring-cyan-200">Sign out</button>
                </form>
            </nav>
        </div>
    </header>
    ...
    <main id="main-content" class="mx-auto max-w-7xl px-5 py-8 sm:px-8">
        ...
        @if ($errors->any())
            <div class="mb-6 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm text-rose-900" role="alert">
